...
public class MakeImage implements MouseListener, MouseMotionListener {
	...
	public MakeImage(String cn) {
		...
		addButton(panel, "SaveAs", new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			//	save(new FileDialog(frame, "", FileDialog.SAVE));
			}
		});
	...
	public void redraw(Graphics g) {
		...
		// canvas.clear();
		g.drawRect(margin - 1, margin - 1, 1 + cols * wd, 1 + rows * wd);
	...
	public void redraw(Graphics g, int r, int c, boolean redisplay) {
		if (image[r][c])
			g.fillRect(margin + c * wd, margin + r * wd, wd, wd);
		else {
			g.clearRect(margin + c * wd, margin + r * wd, wd, wd);
			g.drawRect(margin + c * wd, margin + r * wd, wd, wd);
		}
	}
	...
	private void save(String fname) {
		try {
			if (fname == null) {
				do
					fname = category
							+ ((category == categoryName) ? (++categoryCount)
									: (++otherCount)) + ".pbm";
	...
	public void load() {
		...
		try {
			java.util.regex.Pattern bit = java.util.regex.Pattern
					.compile("[01]");
			Scanner f = new Scanner(new File(new FileDialog(frame).getFile()));
			if (!f.next().equals("P1"))
